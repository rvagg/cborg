...
function decodeFloat (data, pos, minor, options) {
  ...
  if (minor < MINOR_BREAK) {
    throw new Error(`${decodeErrPrefix} unassigned simple values are not supported (${minor})`)
  }

  throw new Error(`${decodeErrPrefix} unknown minor for major 7 (${minor})`)
}
...
function decodeFloat32 (ui8a, pos) {
  return new DataView(ui8a.buffer, (ui8a.offset || 0) + pos, 4).getFloat32(0, false)
}
...
function decodeFloat64 (ui8a, pos) {
  return new DataView(ui8a.buffer, (ui8a.offset || 0) + pos, 8).getFloat64(0, false)
}

...
function argvWidth () {
  const width_keyword = process.argv.findIndex((s) => s === '--width')
  if(width_keyword <= 0 || width_keyword + 1 >= process.argv.length ) {
    return { argv: process.argv.filter((s) => s !== '--width'), width: undefined }
  }
  const width = parseInt(process.argv[width_keyword + 1])
  if(!width || width < 20) {
    return { argv: process.argv.filter((s) => s !== '--width'), width: undefined }
  }
  const argv = process.argv
  argv.splice(width_keyword, 2)
  return { argv: argv, width: width}
}

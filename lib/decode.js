...
// TODO: implement a single-byte jump list for easy elements
function * encodedToTokens (data, options = {}) {
  ...
  while (pos < data.length) {
    ...
    /* c8 ignore next 5 */
    // this is a programming error, there should be 8 decoders and only 8
    // possible values of `data[pos] >>> 5`
    if (!decoder) {
...
function tokensToObject (tokenIter, options) {
  ...
  if (token.type === Type.map) {
    ...
    const obj = options.useMaps !== true ? {} : undefined

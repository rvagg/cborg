...
class JSONEncoder {
  ...
  [Type.string.major] (buf, token) {
    ...
    buf.push(fromString(JSON.stringify(token.value)))
    // buf.push(34) // '"'
...
function mapSorter (e1, e2) {
  ...
  if (keyToken1.type !== Type.string || keyToken2.type !== Type.string) {
    throw new Error('Non-string map keys are not supported')
  }
  return keyToken1 < keyToken2 ? -1 : keyToken1 > keyToken2 ? 1 : 0
}

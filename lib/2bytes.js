...
export function encodeBytes (buf, token) {
  ...
  const prefixBytes = uint.encodeUint(buf, new Token(Type.uint, bytes.length))
...
encodeBytes.encodedSize = function encodedSize (token) {
  const bytes = tokenBytes(token)
  return uint.encodeUint.encodedSize(new Token(Type.uint, bytes.length)) + bytes.length
}

...
function decodeUint (data, pos, minor, options) {
  ...
  if (minor === 27) { // uint64
    // assume BigInt, coerce back to Number if within safe range
...
const uintBoundaries = [24, 2 ** 8, 2 ** 16, 2 ** 32, 2n ** 64n]
...
function encodeUint (buf, token) {
  ...
  if (buint < uintBoundaries[4]) {
    ...
    let lo = Number(buint & 0xffffffffn)
    let hi = Number(buint >> 32n & 0xffffffffn)
    buf.set(8, lo & 0xff)
    lo = lo >> 8
    buf.set(7, lo & 0xff)
    lo = lo >> 8

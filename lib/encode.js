...
import { quickEncode, quickEncodeToken } from './jump.js'
...
const typeEncoders = {
  ...
  boolean (obj) {
    return new Token(obj ? Type.true : Type.false, obj)
  },
  ...
  undefined () {
    return new Token(Type.undefined, undefined)
  },
  ...
  Object (obj, typ, options, refStack) {
    ...
    for (const [key, value] of entriesIter) {
      entries.push([
        ...
        typeof key === 'string' ? typeEncoders.string(key) : objectToTokens(key, options, refStack),
...
function encode (data, options) {
  /*
  const quickBytes = quickEncode(data)
  if (quickBytes) {
    return quickBytes
  }
  */
  ...
  if (!Array.isArray(tokens)) {
    ...
    if (encoder.encodedSize) {
      ...
      if (buf.chunks.length !== 1) {
        throw new Error('chunk bork')
      }
      if (encodedSize !== size) {
        throw new Error(`size bork ${encodedSize} != ${size}`)
      }
      return buf.chunks[0]
    }
  }
...
export { objectToTokens, encode, Ref }

...
function tokensToArray (obj, options, refStack) {
  const arr = []
  for (const t of objectToTokens(obj, options, refStack)) {
    arr.push(t)
  }
  return arr
}
...
const typeEncoders = {
  ...
  * Object (obj, typ, options, refStack) {
    ...
    for (const [key, value] of entriesIter) {
      entries.push([
        tokensToArray(key, options, refStack),
  ...
  * ArrayBuffer (obj) {
    yield new Token(Type.bytes, new Uint8Array(obj))
  },

  * DataView (obj) {
    yield new Token(Type.bytes, new Uint8Array(obj.buffer, obj.byteOffset, obj.byteLength))
  }
}
